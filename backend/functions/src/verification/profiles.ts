    ...
    try {
        if (githubUsername) {
            try {
                ...
                const ghRes = await axios.get(`https://api.github.com/users/${githubUsername}`, { headers });
        ...
        if (stackOverflowId) {
            try {
                const soRes = await axios.get(`https://api.stackexchange.com/2.3/users/${stackOverflowId}?site=stackoverflow`);
                ...
                profileData.stackoverflow = { error: 'Failed to fetch StackOverflow profile.' };
            }
        }
        ...
        await admin.firestore().collection('tutors').doc(request.auth.uid).set({
