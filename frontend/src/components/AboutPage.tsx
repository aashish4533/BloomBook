...
export function AboutPage({ onBack, onNavigateToCommunities }: AboutPageProps) {
  ...
  ]);  // Static, but could fetch from DB

  useEffect(() => {
    // Example: Fetch dynamic stats if needed
    const fetchStats = async () => {
      // e.g., const users = await getDocs(collection(db, 'users')).then(snap => snap.size);
      // But keeping static for now
    };
    fetchStats();
  }, []);
