...
export function DeliveryTracking({ orderId = 'ORD123456', estimatedDelivery = 'Jan 18, 2024', onClose, onBack }: DeliveryTrackingProps) {
  ...
  const activeStep = trackingSteps.find(step => step.status === 'active');

  return (
    <div className="max-w-4xl mx-auto p-4">
      {/* Header Card */}
      <Card className="p-6 mb-6 bg-gradient-to-r from-[#C4A672] to-[#8B7355] text-white">
        <div className="flex items-start justify-between mb-4">
          <div>
            <h2 className="text-2xl mb-2">Delivery Tracking</h2>
            <p className="text-white/90">Order ID: {orderId}</p>
          </div>
          <Badge className="bg-white text-[#C4A672]">
                    ...
                    {index < trackingSteps.length - 1 && (
                      <div
                        className={`absolute left-5 top-12 w-0.5 h-full ${
                          isCompleted ? 'bg-[#C4A672]' : 'bg-gray-200'
                        }`}
                      />
                    )}
                    ...
                    <div className="flex gap-4">
                      ...
                      <div className="flex-1 pb-6">
                        <div className="flex items-start justify-between mb-1">
                          <h4 className={`${
                            isCompleted || isActive ? 'text-[#2C3E50]' : 'text-gray-400'
                          }`}>
                            {step.title}
                          </h4>
                          {isActive && (
